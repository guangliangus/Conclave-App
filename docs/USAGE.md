...
**一句话：PR 的 code review 不再需要有人记得去做。**

- **自己找活。** 它自己去问 Azure DevOps「哪些 PR 是活跃的」，不用谁把 PR 号发给它，
  也就没有「漏掉的那个」。
- **只评没评过的那一版。** 同一版代码不会被评第二遍；作者 push 了修复，自动算成新的
  一版再评一次 —— 而且还是原来那台机器接手，它已经读过这份代码、提过这些问题。
- **活儿分得开。** 哪个 PR 归谁评，是每台机器各自算出来的同一张表，不用互相商量；
  某台超时或跑挂了，下一台自动接手。谁关机都不影响其他人。
- **结论送到你手上。** 评完往 PR 发一条评论加一次投票，同时飞书私聊作者 ——
  不用等谁想起来去翻 PR 页面。
- **过程全程留痕。** 谁在什么时候评了哪一版、结论是什么、提了哪几条问题，都记在一本
  改不了的账本（**Acta**）上，事后随时翻得出来。

还有一档默认关着的：把 quorum 配成 2 或 3，同一个 PR 由多台机器**独立**评一遍再多数决
合并 —— 被三台都提到的问题几乎必然是真问题，只被一台提到的大概率是噪音。
