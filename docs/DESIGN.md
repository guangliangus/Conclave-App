...
要解决的事只有一件：**PR 的 code review 不再需要有人记得去做**。拆开是四条：

| 要做到 | 于是形态上必须有 |
|---|---|
| 不用人报 PR 号，也不会漏 | 每个节点**主动轮询** AzDO 的活跃 PR，而不是等谁把 PR 推给它 |
| 同一版代码不重复评，作者一 push 就重评 | 最小单元是 `Revision = (prId, srcCommit)`，判重的依据是账本本身（§4） |
| 单机不是瓶颈也不是单点 | 评审分散到 mesh 里所有合格节点，谁掉线都有人接手（§6、§7） |
| 结论事后可查、可核对 | 全过程写进签名哈希链账本 Acta，附一层可查投影（§5） |

而「谁评哪个 PR」要在没有服务端、没有调度器的前提下成立，就只剩一条路：
**席位分配必须是纯函数** —— 各节点吃同样的输入（心跳里的成员状态 + 链上的 Summons），
各自算出同一张席位表，谁也不用问谁（§7）。这条约束反过来决定了 Domain 层的形状：
零依赖、不读时钟、不读环境（§12）。

额外拿到的东西：多节点独立评审同一个 PR，**能压掉 LLM 的输出方差** —— 被 3 个节点
独立提到的 finding 几乎必然是真问题，只被 1 个提到的大概率是噪音。这是引入 quorum 的
**唯一**理由（§10）；它是成倍的额度，所以默认只评一次。
...
节点表里每一行画的都是 Claude 自己报的窗口读数（5h / 7d），本节点和对端一样 ——
只有拆不出窗口时才退回画上面那个折算标量。对端那份明细在 `LiveState.UsageWindows` 里，
随 `GET /state` 过来。

**为什么不挂在心跳上。** 心跳走 UDP，而它已经没有余量了：

| 形状 | 心跳字节数 |
|---|---|
| 29 个 project | 1276 |
| 35 个 project | 1418 |
| 35 个 project + 2 条窗口明细 | 1593 ❌ |
| 50 个 project | 1748 ❌ |

以太网 MTU 给 UDP 的净载荷是 1472（1500 − 20 IP − 8 UDP）。两条窗口明细就是 175 字节，
直接把实测形状推过线；而分片的 UDP 是网络设备最爱静默丢的那种包，表现是**整台机器从 mesh
里消失、没有任何报错指向包大小**。走状态那条 HTTP 通道则一点 MTU 预算都不占，还顺带被
`SignedLiveState` 签名保护 —— 挂在心跳上就只能像 `Elector.ClaudeVersion` 那样裸奔
（加进签名载荷 = 全 mesh 同时升级）。这跟队列当初为什么不塞进心跳是同一个理由。

⚠️ **撑满心跳的是 `Elector.Projects`**：35 个名字占了半个包还多，而它随组织规模线性增长 ——
50 个 project 就已经在分片了。`BeaconTests.The_heartbeat_still_fits_in_one_datagram`
钉住了这条线，往心跳里加任何字段之前先跑它。
...
| **契约缺失 / 输出不是 JSON** | 同样出 `Error` 票，但 `Retryable=false` —— **确定性失败，不再重试**（§8.2） |
| **重试到上限仍无有效票** | 拿手上的票收尾 —— 一个 `degraded` 的 `Error` 结论 |
| **`Error` 结论要不要投递** | **不发** —— 对作者没有可行动信息，而且投票本来就是 `none`（§8.3） |
...
重试预算的前提是「换一轮可能就过了」。有两类失败不满足这个前提：**回复末尾没有 collect
契约的 ```json 围栏**，和**输出根本不是合法 JSON**。这两种情况下 claude 已经把评审跑完、
token 也烧光了，只是拿不到机器可读的结论 —— 同一份输入再抽一个席位，得到的是同一个结果。

所以这两条路出的 Error 票标 `BallotPayload.Retryable = false`，`ChainState.HasFatalError`
让 `CanPromulgate` 直接放行，一轮就收尾。

> 实测（PR 2912，2026-09-11）：连着两轮都是「没输出 collect 契约的 JSON 块」，
> 两轮合计 121 万 token、$2.79，产出为零 —— 而两轮的原文里结论、finding、行号全都在，
> 评审本身是好的。按老规则它还会再跑第三轮。

⚠️ **取舍**：skill 是各机器各自装的，所以理论上换一台可能就有能出围栏的版本。仍然一票
否决整个预算 —— 契约是 runner 与 skill 这一对的约定，全 mesh 本来就该同步（版本不齐在
节点表和通知里单独说）；赌「下一台可能不一样」的代价是每个 PR 多烧两轮完整额度，
而判错的代价只是提前拿到一个 degraded 的 Error 结论，人看到原因重跑即可。

**契约在两个地方说**：`ClaudeReviewRunner.CollectPrompt` 里那句 prompt，和 skill 的第 7 步。
刻意重复 —— 解析它的是 runner，把「出票能不能成」整个押在一个一万多字的外部文档的末尾
几行上，失败起来是静默的。

### 8.3 执行失败不往 PR 上发

`degraded` 的 `Error` 结论不调 `PostResultAsync`。它对作者**没有任何可行动信息**：
「评审没跑出结论」是这边的运维问题（skill 不出契约块、claude 起不来、工作区拉不下来），
不是他代码的问题；而且 `AzVote` 对 `Error` 本来就返回 `none`，发了也不改变 PR 状态。

留痕一点没少：链上照常写 `Promulgation`，本地照常出一条 `NoticeKind.Bad` 的通知（带原因），
日志照常记 Warning。「这个 PR 评过、但没评出来」在本机查得到。

> 渲染那条路也修了：零有效票时 `Findings.Count == 0`，而 `RenderComment` 原先只看这一个
> 条件就写「未发现问题。」—— 「跑完了，是干净的」和「压根没跑出结论」对作者的意义
> 正好相反。同时「降级：合格节点不足」也是硬编码的，现在两种降级原因分开说。

**围栏按行判定，不是 `IndexOf("```")`。** 契约里的 `comment` 是要发到 PR 上的评论原文，
而一条像样的评审评论几乎必然带 ```` ```go ```` 这样的示例代码 —— 那些反引号在 JSON
字符串**里面**。老实现拿 `IndexOf` 一路配对，会把其中第一个当成收尾围栏，抠出来的是
一段从中间截断的 JSON，于是「评论写得越好越解不出来」。

> 实测（PR 2916，2026-09-12）：reject + 18 条 finding + 5426 字的评论原文全都在，
> 就因为原文里有七段 `go` 示例，解析出 `Error` 票，$1.31 烧光，作者在 PR 上收到的是
> 一句「评审没有跑出结论」。

按行判定靠的是一条硬性质：**JSON 字符串里不可能有真换行**（只能是 `\n` 两个字符），
所以字符串里的 ``` 一定在行中间，而 markdown 的围栏一定独占一行。收尾围栏不带
info string（CommonMark），带了的按块内内容算。开了却没收尾的（模型漏写、输出被
max-tokens 截断）再补一次带字符串状态的花括号配平兜底 —— 这一轮的账单反正已经烧掉了。
...
2026-09-17 在 `cli_a952edf34c38de17`（组里现有的那个飞书自建应用）上连打三次：

| 调用 | 结果 |
|---|---|
| `im/v1/messages`，`receive_id_type=email`，发给应用开发者本人 | ✅ `om_x100b6582dd18eca0ee82e60ef4a0e21` |
| `contact/v3/users/batch_get_id`，邮箱换 open_id | ❌ 99991672：**未申请** `contact:user.id:readonly` |
| `im/v1/messages`，`receive_id_type=email`，发给一个普通同事 | ✅ 回的是另一个 `chat_id`，确是另一个人 |

第二条跟另外两条互为反证：**这个应用没有通讯录权限，而邮箱直投照样成功**。所以
`receive_id_type=email` 的收件人解析是租户侧做的，不经过应用自己的通讯录可见性。

这一节早先记的是完全相反的结论（「只有 open_id 真的能发」「邮箱一律 99992402」），
连机制解释（应用读不到 `email` 字段所以解析不出来）也是错的。特意把更正写在这里，
是因为那个错误结论已经抄进了 `LarkOptions` 与 `LarkNotifier` 的注释 —— 下一个人
多半是从文档反推代码的。

### 真正的闸是「可用范围」，不是 scope

99992402 同时表示「这个人不存在」和「收件人解析不出来」，单看没有信息量。早先那次测试
大概率就是撞上了可用范围不含收件人的应用，然后把原因归到了 scope 上。

所以换一个应用重来时，唯一要确认的是**可用范围覆盖到要收通知的人**，而不是权限列表。
那是这条路唯一的静默失效点：范围外的人发不出去，返回的还是同一个 99992402。上面第三次
调用（发给一个普通同事而不是开发者自己）验的就是这件事 —— 开发者天然在可用范围内，
只发给自己是测不出这道闸的。

### 三跳降级链的实际走法

`LarkNotifier` 把收件人解析写成三跳，但在不开通讯录权限的应用上只会落到最后一跳：

| 跳 | 实际 |
|---|---|
| `UserMap` 里写死 `ou_…` | 只在显式配了的人身上命中，默认没人配 |
| 邮箱换 open_id | 99991672 → `_canResolveOpenId` 锁死，**整个进程只白打这一次** |
| `receive_id_type=email` 直投 | **常态路径，而且是通的** |

99991672（scope 未申请）在一个进程生命周期内不会变 —— 开权限要去开发者后台，改完得重启
节点 —— 所以只探一次。想连这一次都省掉就去开 `contact:user.id:readonly`，不开也只多一个
请求，没有别的代价。
...
分阶段落地的顺序。每一阶段的验收标准是「到这里停下也是有用的」：

| 阶段 | 内容 | 到这里就能拿来干什么 |
|---|---|---|
| P0 | Avalonia 壳 + az 轮询 + 本地 Acta + 手动触发 review | 一台机器上把「看见 PR → 评一遍 → 留痕」跑通 |
| P0.5 | `srcCommit` 幂等键：作者一 push 自动重跑，不 push 不重跑 | 活跃 PR 自动发现 + 去重，不漏不重 —— 单机就已经独立创造价值 |
| P1 | mesh 发现 + 签名心跳（带 `Projects` / `AzIdentity` / `RunningJobs` / 额度） | 几台机器互相看得见彼此的负载与权限 |
| P2 | Block gossip + 增量补链 + 索引冲突让位重挂 | 账本在多台之间收敛成一条链 |
| P3 | project 分片轮询 + 硬规则过滤 + 加权 HRW 认领 | 活儿真的分散开，谁掉线都有人接手 |
| P4 | quorum 自适应 + findings 合并 + 单节点投递 | 多节点独立评审压掉方差，而 PR 上只出现一条评论 |
| ~~P5~~ | ~~飞书 gateway（被动等消息触发评审）~~ —— 主动轮询比被动等消息更完整；「手上只有一个 PR 号、想插队」这条需求由无头入口 `review <id>` 覆盖 | ❌ 砍掉 |
