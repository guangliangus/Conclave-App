...
| `seven_day` | `min(0.95, (已过工作日 + 1) / 总工作日)` |
| `seven_day:<模型>` | 不判定 |

周额度那条读作：**一周的额度给五个工作日花，到第 N 个工作日就该只花掉 N/5，允许超前一个
工作日。** 已过工作日按小时平滑计入，不是整天跳一格 —— 否则每天 0 点会出现一道阶跃。
0.95 是硬顶：配速到周末会涨到 1.0，没有它的话一台用掉 99% 的机器仍会被判成「还能接」。
...
另一个副作用是好的：被节流的节点**不用少烧一点就能自己走出来** —— 用量不动、日子往前走，
阈值自己涨上去。旧规则下那条曲线是平的，一旦越线只能等重置。
