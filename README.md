...
**确定性失败一轮就收尾，不把预算烧完**（`BallotPayload.Retryable`）。分两类：

| Error 的原因 | 还重试吗 |
|---|---|
| 工作区拉不下来、子进程挂了、服务端 5xx、评审超时 | **重试** —— 换一轮换一台很可能就过了 |
| 回复末尾没有 collect 契约的 ```json 围栏、输出根本不是合法 JSON | **不重试** |

后一类里 claude 其实<b>把评审做完了</b>、token 也烧光了，只是拿不到机器可读的结论；
同一份输入再抽一个席位，跑出来的是同一个结果。实测一个 PR 连烧两轮 121 万 token、
$2.79，产出为零 —— 而那两轮的原文里结论、finding、行号全都在。
...
| claude 子进程失败 / 超时 | 写 `Ballot` 且 `decision=error` + stderr 摘要。**该轮算烧掉、换人重试**，不作为结论 |
| 契约缺失 / 输出不是 JSON | 同样出 `Error` 票，但标 `Retryable=false` —— **不再重试**，直接收尾（重试只会原样再失败一次，而每轮都是完整账单） |
| 重试到 `MaxReviewAttempts` 仍无有效票 | 拿手上的票收尾 —— 一个 `degraded` 的 `Error` 结论，链上留痕 |
| `Error` 结论的投递 | **不发到 PR** —— 「评审没跑出结论」是运维问题，对作者没有可行动信息（投票本来也是 `none`）。链上与本地通知照常留痕 |
...
六张表从左到右是：**在线节点**（额度 / 在跑 / 近 24H —— 正好是席位权重的三个输入；每行小字里
还有那台机器的 Conclave 版本和 claude 版本，混合版本的集群里这两个值是「为什么只有它评不出来」
「为什么它的判断跟别人不一样」的第一现场，tooltip 里还会写明跟本节点是不是同一版）、
**我的 PR**（队列里作者是本机 az 身份的那些 —— 本节点评不了它们，只能指派出去）、
**PR 队列**（幂等键 / 阶段与收票进度 / 本节点席位 / 结论 + 手动触发与指派按钮，也是启动时默认
停在的那张）、**评审记录**（账单，含缓存命中与分模型明细）、**Acta 会议录**（区块浏览器）、
**通知**（见下）。
...
「在线节点」表里那一列同样**每台机器都画 5h / 7d 的真实读数**，不是本机一种画法、
别人一种画法。对端那份明细随 `GET /state` 过来（`LiveState.UsageWindows`），不进心跳 ——
心跳走 UDP，实测 35 个 project 就已经 1418 字节、离 MTU 只剩 54 字节。拆不出窗口时
（真值读不到，或对端还是旧版本）那一行才退回画折算出来的「压力」标量，标签上写明。
